...
const setExistingStorageCapacity = (request) => {
  if (!!getYarValue(request, "applicantType")) {
    const appType = getYarValue(request, "applicantType");
    // const limit = getYarValue(request, "existingStorageLimit");
    const exisitingStorageLimit = appType === "Pig" ? 8 : 6;
    setYarValue(request, "existingStorageLimit", exisitingStorageLimit);
    console.log("here: ", getYarValue(request, "existingStorageLimit"));
  }
};
...
const sendContactDetailsToSenders = async (request, confirmationId) => {
  try {
    ...
    // TODO: Replace with new HAPI-GAPI
    ...
    return {
      ...
      title: title.replace(
        ...
        (_ignore, additionalYarKeyName) => {
          // console.log("------- additionalYarKeyName: ", additionalYarKeyName);

          return formatUKCurrency(
            getYarValue(request, additionalYarKeyName) || 0
          );
        }
      ),
    };
...
const parseAnswers = (question, request) => {
  ...
  question.answers.map((answer) => {
    ...
    answer.value = answer?.value.replace(
      SELECT_VARIABLE_TO_REPLACE,
      (_ignore, additionalYarKeyName) => {
        const value = getYarValue(request, additionalYarKeyName);
        console.log(
          "-------oldValue: ",
          additionalYarKeyName,
          "New Value:  ",
          value
        );
        return value;
