...
describe("Page: /existing-storage-capacity", () => {
  ...
  it("page loads successfully, with all the options", async () => {
    ...
    expect(response.payload).toContain(
      "8 months or more, but it is no longer fit for purpose"
    );
    expect(response.payload).toContain(
      "8 months or more, and it is fit for purpose"
    );
  });
  ...
  it("no option selected -> show error message", async () => {
    ...
    expect(postResponse.payload).toContain(
      `Select how many months&#39; slurry storage capacity you have`
    );
  });

  it.only("user selects ineligible option: '8 months or more' -> display ineligible page", async () => {
    const postOptions = {
      method: "POST",
      url: `${global.__URLPREFIX__}/existing-storage-capacity`,
      headers: { cookie: "crumb=" + crumbToken },
      payload: {
        // existingStorageLimit: 8,
        applicantType: 'Pig',
        existingStorageCapacity: "8 months or more, and it is fit for purpose",
        crumb: crumbToken,
      },
    };
  ...
  it("user selects eligible option -> store user response and redirect to /planned-storage-capacity", async () => {
    const postOptions = {
      ...
      headers: { cookie: "crumb=" + crumbToken },
      payload: {
        existingStorageCapacity: "Less than 8 months",
        crumb: crumbToken,
      },
    };
  ...
  it("page loads with correct back link", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.payload).toContain(
      '<a href="system-type" class="govuk-back-link">Back</a>'
    );
  });
});
