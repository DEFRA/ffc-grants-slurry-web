...
it("page loads successfully, with all the options", async () => {
    ...
    expect(response.payload).toContain("Is the existing store you want to cover fit for purpose?");
...
it("no option selected -> show error message", async () => {
    ...
    expect(postResponse.payload).toContain("Select if your existing store you want to cover is fit for purpose");
});
...
it("user selects eligible option: \'No\' when applying-for page is Not impermeable cover only  -> store user response and redirect to /estimated-grant", async () => {
    varList.applyingFor = 'Building a new store, replacing or expanding an existing store'
    varList.projectType = 'Replace an existing store that is no longer fit for purpose with a new store'
    varList.grandFundedCover = 'Yes, I need a cover'
    varList.existingCover = 'Yes'
...
it("page loads with correct back link", async () => {
    ...
    const response = await global.__SERVER__.inject(options);
    expect(response.statusCode).toBe(200);
    expect(response.payload).toContain('<a href="existing-cover-pig" class="govuk-back-link">Back</a>');
});
...
it("page loads with correct back link", async () => {
    ...
    const response = await global.__SERVER__.inject(options);
    expect(response.statusCode).toBe(200);
    expect(response.payload).toContain('<a href="existing-cover" class="govuk-back-link">Back</a>');
});
